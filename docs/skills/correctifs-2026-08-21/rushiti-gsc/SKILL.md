...
|---|---|
| Requêtes en position 8-20 avec impressions réelles (ce qui peut monter) | `rushiti-opportunites-gsc` (mode 1 — striking distance) |
| Bien classé mais peu cliqué — title/meta à réécrire | `rushiti-opportunites-gsc` (mode 2 — CTR) |
...
| 404, chaînes de redirections, canonicals, titles dupliqués à l'échelle du site | `rushiti-audit-site` (mode 2 — crawl autonome) |
| Core Web Vitals mesurés, crawlabilité, robots IA, audit technique complet | `rushiti-audit-site` |
...
4. **robots.txt.** Vérifier qu'il est servi en 200 et qu'il ne bloque ni Googlebot ni le sitemap (ligne `Sitemap:` présente). Le diagnostic complet des robots (y compris robots IA) appartient à `rushiti-audit-site`.
...
1. **Performance — Résultats de recherche.** Période : 3 derniers mois **avec comparaison** à la période précédente. Relever : clics, impressions, CTR moyen, position moyenne, et le top requêtes/pages. Questions à trancher :
   ...
   - Des positions 8-20 à fort volume ? → routage `rushiti-opportunites-gsc` (mode 1).
   - Un CTR anormalement bas pour une bonne position ? → routage `rushiti-opportunites-gsc` (mode 2).
...
3. **Expérience — Core Web Vitals et HTTPS.** Relever le nombre d'URL « Bonnes / À améliorer / Médiocres », **mobile d'abord** — la majorité des clients cherchent un artisan depuis leur téléphone. Toute URL « Médiocre » ou tout problème HTTPS → routage `rushiti-audit-site` (qui mesure via PageSpeed Insights). Petit site : si GSC affiche « Pas assez de données », le dire tel quel — ce n'est ni bon ni mauvais signe.
...
| Page avec redirection | L'URL redirige, c'est la cible qui compte | Vérifier que la cible est la bonne ; chaînes → `rushiti-audit-site` |
| Autre page avec balise canonique correcte | Doublon assumé, la canonique est indexée | Généralement rien à faire |
| Introuvable (404) | Page supprimée ou lien cassé | Si la page a de la valeur : rediriger en 301 → `rushiti-audit-site` |
...
|---|---|
| `rushiti-opportunites-gsc` (mode 1), `rushiti-cannibal-check` | Performance → filtre 3-6 mois → **croisé requête × page** (onglet Requêtes, puis clic sur une requête → Pages ; ou export API/Looker) — à défaut, onglets Requêtes et Pages séparés, en le signalant |
| `rushiti-regression-seo` | Deux périodes **comparables** (mêmes durées, même saison) ou l'export avec comparaison activée |
| `rushiti-opportunites-gsc` (mode 2) | Performance → Pages et Requêtes avec clics, impressions, CTR, position |
...
- **Analyser ici ce qu'un skill spécialiste fait mieux.** Une demi-analyse CTR dans le check-up contredira l'analyse complète de `rushiti-opportunites-gsc`. Détecter, router, s'arrêter.
...
- « peintre besançon » — pos. 9,8, 2 100 impr., 31 clics : striking distance au cœur
  du métier → rushiti-opportunites-gsc
- « dégât des eaux plafond qui paye » — pos. 14,6, 1 350 impr., 4 clics : requête
  ...
  → rushiti-opportunites-gsc (et bloc FAQ probable → rushiti-faq)
...
- 12 pages indexées (stable). 2 « Page avec redirection » (anciennes URL — vérifier que
  les cibles sont les bonnes → rushiti-audit-site) ; 1 « Détectée, actuellement non
...
## Décisions du mois
1. Lancer rushiti-opportunites-gsc sur l'export croisé (à préparer : Performance,
   3 mois, requête × page) — cible : « peintre besançon » et la requête sinistres.
2. Vérifier les cibles des 2 redirections via rushiti-audit-site.
