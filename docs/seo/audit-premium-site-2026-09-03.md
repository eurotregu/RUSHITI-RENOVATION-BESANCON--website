...
*Constat 25, charte (04/09, 12 h 42 UTC) : décision d'Isuf = variante B (bleu nuit `#002B4B` et vert `#016738` de la charte, orange d'action conservé). Paquet 14 fusionné en production (PR [#45](https://github.com/eurotregu/rushiti-renovation/pull/45)), déploiement Pages du commit de fusion réussi, cache Cloudflare purgé sur accord écrit. Contrôle en direct de l'accueil et de `/peinture-interieure-besancon` : `theme-color #002B4B`, variables `--navy`, `--navy-dark`, `--ok` de la variante B dans la CSS inlinée, coches SVG `#016738`, aucun résidu des anciennes couleurs, orange inchangé. Reste pour la charte v2.8 : inscrire l'orange `#E8743B` comme couleur d'action et le bleu nuit foncé `#001E36`, et traiter le contraste blanc/orange des boutons (3,0:1).*

*Recontrôle (04/09, après-midi) : `/peinture-interieure-besancon` relue en direct à la demande d'Isuf : conforme (aucun `div.stars`, « relevé le 22/08/2026 », pas d'`aggregateRating`, `legalName` Rushiti, bandeau GA4, CSS p0 et p12 servies, ni plomberie ni électricité). Écart secondaire relevé au passage : six pages fixes sans bloc `geo` (le paquet 12 ne remplaçait que les blocs existants) → paquet 13 `korrigjime-prodhim/paketa-13-geo-pages-fixes/`, PR de production à relire.*

*Déploiement (04/09, 12 h 14 UTC) : PR #42 fusionnée par Isuf, Worker envoyé par l'API (version 42, `2026-09-04-avis-sans-etoiles`, 100 % du trafic) après accord écrit. Contrôle en direct : `/peinture-interieure-besancon` sans `div.stars`, « 34 avis Google · relevé le 22/08/2026 » présent. Le code en ligne contenait un bloc « bailleur privé » du 02/09 (page remise en état locatif) absent du dépôt : conservé, vérifié en ligne (FAQ DPE et lien assurance toujours servis) et repris dans la copie versionnée → PR [#43](https://github.com/eurotregu/rushiti-renovation/pull/43) (brouillon).*
